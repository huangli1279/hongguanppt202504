...
const InsightBox = ({ title, children, icon: Icon, delay, alert }: { title: string, children?: React.ReactNode, icon: any, delay: string, alert?: boolean }) => (
  <div className={`flex flex-col h-full p-4 bg-slate-50 border-t-2 ${alert ? 'border-red-500' : 'border-mckinsey-blue'} opacity-0 animate-fade-in-up fill-mode-forwards`} style={{ animationDelay: delay }}>
    <div className={`flex items-center gap-2 mb-2 ${alert ? 'text-red-600' : 'text-mckinsey-blue'}`}>
      <Icon size={18} className={alert ? "" : "text-mckinsey-accent"}/>
    ...
    <div className="w-full h-full bg-white flex flex-col p-12 overflow-hidden relative">
      ...
      <header className="mb-6 animate-fade-in">
        ...
        <h1 className="text-3xl font-serif font-bold text-mckinsey-blue leading-tight w-11/12">
          ...
          <span className="text-mckinsey-subtext">内需修复斜率放缓</span>
        </h1>
      </header>
