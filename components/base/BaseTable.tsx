...
export const BaseTable: React.FC<BaseTableProps> = ({
  ...
  striped = true,
  bordered = false,
  headerBgColor = '#051c2c', // = webank.blue
  ...
  if (isAuto) {
    return (
      <div className="w-full h-full flex flex-col" role="table" aria-label={typeof title === 'string' ? title : undefined}>
        {(title || subtitle) && (
  ...
  // 非auto模式：使用传统table
  return (
    <div className="w-full h-full flex flex-col" role="region" aria-label={typeof title === 'string' ? title : undefined}>
      {(title || subtitle) && (
