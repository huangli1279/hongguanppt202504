...
const ChevronArrow: React.FC<{ color: string }> = ({ color }) => (
  <div className="flex items-center justify-center flex-shrink-0 w-3.5">
    <svg viewBox="0 0 20 40" className="w-3 h-6" fill="none">
      ...
      <div className="flex flex-col h-full">
        <div className="mb-2 flex-shrink-0">
          ...
          <div className="mb-1.5">
            <BaseCard delay="0ms" variant="accent" className="!p-3 gap-1">
          ...
          <div className="flex items-stretch gap-0">
            {chainNodes.map((node, i) => (
              <React.Fragment key={node.category}>
                <div
                  className={`flex-1 rounded-md overflow-hidden ${node.bgColor} border border-gray-100 shadow-sm
                    animate-fade-in-up fill-mode-forwards opacity-0`}
                  style={{ animationDelay: `${i * 120}ms` }}
                >
                  <div className={`${node.headerColor} text-white text-center text-[9px] font-semibold py-0.5 px-0.5`}>
                    {node.category}
                  </div>
                  <div className="flex flex-col items-center justify-center py-1 px-0.5">
                    <span className={`text-sm font-bold ${node.textColor} leading-none`}>
                      {node.percentage}
                    </span>
                    <div className="mt-0.5 text-center">
                      {node.lines.map((line, j) => (
                        <p key={j} className="text-[8px] text-gray-500 leading-tight">{line}</p>
                      ))}
                    </div>
                  </div>
                </div>
