...
import { gdpContributionData } from '@/data';
import { Globe, Building, ShoppingCart } from 'lucide-react';

// 贡献率配色
const contributionColors = {
  consumption: '#005c8f',  // 消费 - 深蓝（压舱石）
  investment: '#ef4444',   // 投资 - 红色（拖累）
  netExport: '#10b981',    // 净出口 - 绿色（支撑）
};
...
export const ContentSlide07: React.FC = () => {
  return (
    <BaseContentSlide
      ...
      cards={
        <>
          <BaseCard title="外需：强力支撑" icon={Globe} delay="100ms" variant="accent">
            <p>
          ...
          <BaseCard title="投资：明显拖累" icon={Building} delay="200ms">
            <p>
          ...
          <BaseCard title="消费：压舱石" icon={ShoppingCart} delay="300ms">
            <p>
      ...
      charts={
        <ChartContainer delay="500ms">
          <BaseStackedBarChart
            data={gdpContributionData}
            title="2024-2025年GDP当季同比贡献率：三大需求"
            subtitle="单位: %"
            bars={[
              { dataKey: 'consumption', name: '最终消费支出', color: contributionColors.consumption },
              { dataKey: 'investment', name: '资本形成总额', color: contributionColors.investment },
              { dataKey: 'netExport', name: '货物和服务净出口', color: contributionColors.netExport },
            ]}
